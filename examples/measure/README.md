...
For each `test_case_i`, `measure` writes outputs directly inside the case
directory.

The main outputs are:

- `continuous.joblib`: continuity flag for the track,
- `row_statistics.csv`: row-level geometry and porosity information,
- `cross_sections_statistics.csv`: cross-section-level W, H, D, and porosity,
- `images_full_meltpool/`: diagnostic plots generated from the extracted
  metrics.
